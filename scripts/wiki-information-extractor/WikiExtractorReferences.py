#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Input:  .xml dump of Wikipedia as provided by Wikimedia project
# Output: 3 column tsv file: <wikipedia-id> <reference-count> <title>
# Sample run: ./WikiExtractorReferences.py --processes 4 -o <output dir path> -b 2M -ns 'w' -it abbr,b,big --filter_disambig_pages --no-templates --discard_elements gallery,timeline,noinclude <input .xml file>
# =============================================================================
#  Version: 2.75 (March 4, 2017)
#  Author: Giuseppe Attardi (attardi@di.unipi.it), University of Pisa
#  Modified by: Dwaipayan Roy (dwaipayan.roy@gmail.com)
#
#  Contributors:
...
class Extractor(object):
    ...
    def write_output(self, out, text):
        ...
        if options.write_json:
            ...
            refCount = 0;
            for line in text:
                refCount += line.count("</ref>")
                if out == sys.stdout:   # option -a or -o -
                    line = line.encode('utf-8')
            #out.write(str(refCount).encode("utf-8").decode("utf-8")+"\t")
            out.write('%s\t%s\n' % (str(refCount).encode("utf-8").decode("utf-8"), self.title))
            #out.write(footer)
    ...
    def wiki2text(self, text):
        ...
        text = text.replace("'''", '').replace("''", '"').replace("\n ", "\n") # .replace("\n ", "\n") is added by Dwaipayan (07-09-18)
