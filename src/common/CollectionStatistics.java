...
public class CollectionStatistics {
    ...
    public CollectionStatistics(IndexReader indexReader) {
 ...
 vocSize       - collection-size<p>
    ...
    public void getQueryDocumentVector(int luceneDocid, String[] qTerms, float lambda) throws IOException {
        ...
        Terms terms = indexReader.getTermVector(luceneDocid, FIELD_CONTENT);
        if(null == terms) {
...
//        (float)Math.log(1 + ((1 - lambda) * freq / docLen) /(lambda * ((LMStats)stats).getCollectionProbability()));
     ...
     * @param luceneDocid The lucene doc id of the corresponding document
     * @param indexReader The index reader
     * @param fieldName The name of the field for which the document vector will be computed
     * @throws IOException 
     */
    public void showDocumentVector(int luceneDocid, IndexReader indexReader, String fieldName) throws IOException {
...
//            + " cf: " + colFreq*/);
    ...
    public void allDocLength() throws IOException {
        ...
        for (int i=0; i<indexReader.maxDoc(); i++) {
            ...
            // Term vector for this document and field, or null if term vectors were not indexed
            Terms terms = indexReader.getTermVector(i, FIELD_CONTENT);
            if(null == terms) {
                fullLen = 0;
            }
            ...
            // Term vector for this document and field, or null if term vectors were not indexed
            terms = indexReader.getTermVector(i, FIELD_CONTENT);
            if(null == terms) {
                cleanLen = 0;
            }
            else {
